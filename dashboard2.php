...
$top_mixes = $db->query("SELECT id, title, dj, downloads, plays FROM mixes ORDER BY downloads DESC LIMIT 10")->fetchAll();
$top_djs_stats = $db->query("SELECT dj, SUM(downloads) as total_downloads, SUM(plays) as total_plays FROM mixes GROUP BY dj ORDER BY total_downloads DESC LIMIT 5")->fetchAll();
        ...
        <main class="main-content">
            ...
            <div id="dashboard-section" class="section-content">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
                    ...
                    <div class="bg-neutral-900 rounded-xl p-4"><h3 class="text-lg font-semibold mb-3">🏆 Top 5 DJs más Descargados</h3><div class="space-y-2"><?php foreach ($top_djs_stats as $dj): ?><div class="flex justify-between items-center p-2 hover:bg-neutral-800 rounded"><div><p class="font-medium text-sm"><?php echo htmlspecialchars($dj['dj']); ?></p></div><div class="text-right"><p class="text-primary font-bold"><?php echo number_format($dj['total_downloads']); ?></p></div></div><?php endforeach; ?></div></div>
                </div>
                    ...
                    <div class="table-container">
                        <table class="w-full">
                            ...
                            <tbody id="users-table">
                                ...
                                <td><button onclick="showChangePasswordModal(<?php echo $user['id']; ?>, '<?php echo htmlspecialchars($user['username']); ?>')" class="text-yellow-400 mr-1"><i class="fas fa-key"></i></button><button onclick="deleteUser(<?php echo $user['id']; ?>)" class="text-red-400"><i class="fas fa-trash"></i></button></td></tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                    </div>
