...
if ($beforeId > 0) {
    $cursorCondition = 'AND m.id < :before_id';
} elseif ($afterId > 0) {
    $cursorCondition = 'AND m.id > :after_id';
    $order = 'ASC';
    $limit = 81;
}
...
$pageSize = $afterId > 0 && $beforeId === 0 ? 80 : 50;
$hasMore = count($messages) > $pageSize;
if ($hasMore) {
    $messages = array_slice($messages, 0, $pageSize);
}
if ($order === 'DESC') {
...
chat_json(['success' => true, 'messages' => $messages, 'has_more' => $hasMore, 'server_time' => date('c')]);
?>
