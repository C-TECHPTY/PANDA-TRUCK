...
function store_ensure_tables(PDO $db): void
{
...
function store_absolute_url(?string $path): string
{
...
function store_order_code(): string
{
...
function store_notify_order(PDO $db, int $orderId): void
{
    ...
    $html = '<div style="font-family:Arial,sans-serif;color:#222;">'
        ...
        . '<p><strong>Direccion/nota:</strong><br>' . nl2br(htmlspecialchars(trim(($order['customer_address'] ?? '') . "\n" . ($order['customer_note'] ?? '')))) . '</p>'
        . $receipt
